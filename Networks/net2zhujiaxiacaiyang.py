...
class MODEL(nn.Module):
    def __init__(self, img_size=120, patch_size=4, embed_dim=96, in_channel=2, out_channel=8, output_channel=1,
        ...
        self.up1 = nn.ConvTranspose2d(8, 8, kernel_size=4, stride=4)
        self.convolutional_out = Basic1x1(16, 1)
        self.acmix_net = ACmix(2, 24, stride=1, dilation=1)
        self.acmix_net1 = ACmix(8, 8, stride=1)
    ...
    def forward(self, input):
        ...
        convInput = self.acmix_net(input)  # 1,16,120,120
        ...
        layer2 = self.acmix_net1(self.acmix_net1(layer2))
        ...
        layer3 = self.acmix_net1(self.acmix_net1(self.acmix_net1(layer3)))
        ...
        add2 = self.up(add2)  ### 10,32,120,120
...
if __name__ == '__main__':
    ...
    flops, params = profile(model, inputs=(input,))
    print(f"FLOPs: {flops / 1e9:.2f} GFLOPs11111111")
    print(f"Parameters: {params / 1e6:.2f} M")

    # 使用 torchsummary 计算参数数量
    summary(model, input_size=(2, 256, 256))

    # 使用 torchinfo 计算参数数量
    info_summary(model, input_size=(1, 2, 256, 256))
